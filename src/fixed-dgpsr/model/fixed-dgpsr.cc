...
#define DGPSR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("DGpsrRoutingProtocol");
...
// TODO read ns3 api
        ...
        {
                static TypeId tid = TypeId ("ns3::dgpsr::DeferredRouteOutputTag").SetParent<Tag> (); //親のtidを設定し、<Tag>を取得するのはテンプレート
                return tid;
        }
...
/// 最大許容ジッター
#define DGPSR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// [(-GPSR_MAXJITTER)-GPSR_MAXJITTER] の間の乱数は、HELLO パケット送信のジッターに使用される
#define JITTER (Seconds (x->GetValue (-DGPSR_MAXJITTER, DGPSR_MAXJITTER)))
#define FIRST_JITTER (Seconds (x->GetValue (0, DGPSR_MAXJITTER))) //最初のHelloは過去のものではなく,SetIpv4でのみ使用



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// IANAではまだ定義されていない、DGPSR制御トラフィックのUDPポート
const uint32_t RoutingProtocol::DGPSR_PORT = 666;
...
{
        static TypeId tid = TypeId ("ns3::dgpsr::RoutingProtocol")
                            ...
                            .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                                           EnumValue (DGPSR_LS_GOD),
                                           MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                                           MakeEnumChecker (DGPSR_LS_GOD, "GOD",
                                                            DGPSR_LS_RLS, "RLS"))
                                  ...
                                  LocalDeliverCallback lcb, ErrorCallback ecb)
{
        ...
        {
                NS_LOG_LOGIC ("No dgpsr interfaces");
                return false;
        }
        NS_ASSERT (m_ipv4 != 0);
        NS_ASSERT (p != 0);
        ...
        if (m_ipv4->IsDestinationAddress (dst, iif))
        {
                ...
                TypeHeader tHeader (DGPSRTYPE_POS);
                //パケットからパケットヘッダを取り除く
                packet->RemoveHeader (tHeader);
                //この部分は書かれていないので、オールバリッドとして扱われる
                if (!tHeader.IsValid ())
                {
                        NS_LOG_DEBUG ("DGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
        ...
        if(recovery)
        {
                ...
                while(m_queue.Dequeue (dst, queueEntry))
                {
                        ...
                        if (!tHeader.IsValid ())
                        {
                                NS_LOG_DEBUG ("DGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
        ...
        //dgpsrは1つのインターフェイスに1つのIPアドレスしか持てません
        if (l3->GetNAddresses (interface) > 1)
        {
                NS_LOG_WARN ("DGPSR does not work with more then one address per each interface.");
        }
        ...
        NS_ASSERT (socket != 0);
        socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvDGPSR, this));
        socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), DGPSR_PORT));
...
//socketのパケットを受信する
void
RoutingProtocol::RecvDGPSR (Ptr<Socket> socket)
{
        ...
        TypeHeader tHeader (DGPSRTYPE_HELLO);
        
        packet->RemoveHeader (tHeader);
        if (!tHeader.IsValid ())
        {
                NS_LOG_DEBUG ("DGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                NS_LOG_DEBUG ("RecvDGPSR meet error");
                return;
        }
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("DGPSR does not work with more then one address per each interface. Ignore added address");
        }
}
//アドレスの削除
void
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("Remove address not participating in DGPSR operation");
        }
}
...
//Hello Packetsの送信
void
RoutingProtocol::SendHello (md_ctx_ip, md_ctx_pos)

{
        ...
        std::cout << "postionX is: " << positionX << std:endl;
        ...
        // 署名作成（位置）
        unsigned char digest1[SHA256_DIGEST_LENGTH];//ハッシュ値計算
        SHA256(reinterpret_cast<const unsigned char*>(positionX.c_str()), positionX.length(), digest1);
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
	{
		        ...
		        packet->AddHeader (helloHeader);
                        ...
                        socket->SendTo (packet, 0, InetSocketAddress (destination, DGPSR_PORT));
                }
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
        ...
        NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid DGPSR source address not found");
