...
#define LGPSR_LS_GOD 0

#define LGPSR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("LGpsrRoutingProtocol");
...
// TODO read ns3 api
        ...
        {
                static TypeId tid = TypeId ("ns3::lgpsr::DeferredRouteOutputTag").SetParent<Tag> (); //设置parent的tid,获取<Tag>是模版
                return tid;
        }
...
#define LGPSR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-LGPSR_MAXJITTER)-LGPSR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (x->GetValue (-LGPSR_MAXJITTER, LGPSR_MAXJITTER)))
#define FIRST_JITTER (Seconds (x->GetValue (0, LGPSR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for LGPSR control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::LGPSR_PORT = 666;
...
{
        static TypeId tid = TypeId ("ns3::lgpsr::RoutingProtocol")
                            ...
                            .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                                           EnumValue (LGPSR_LS_GOD),
                                           MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                                           MakeEnumChecker (LGPSR_LS_GOD, "GOD",
                                                            LGPSR_LS_RLS, "RLS"))
                                  ...
                                  LocalDeliverCallback lcb, ErrorCallback ecb)
{
        ...
        {
                NS_LOG_LOGIC ("No lgpsr interfaces");
                return false;
        }
        NS_ASSERT (m_ipv4 != 0);
        NS_ASSERT (p != 0);
        ...
        // TypeHeader tHeader (LGPSRTYPE_POS);
        // //去掉packet的包头
        ...
        if (m_ipv4->IsDestinationAddress (dst, iif))
        {
                ...
                if (!tHeader.IsValid ())
                {
                        NS_LOG_DEBUG ("LGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                        NS_LOG_DEBUG ("RoutingInput meet error");
                        return false;
                }
                NS_LOG_DEBUG ("Received packet");
                //如果是POS的包，就把POS的包头再去掉，所以不需要了解pos的信息了，直接去掉）
                if (tHeader.Get () == LGPSRTYPE_POS)
                {
                        PositionHeader phdr;
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
        ...
        if(recovery)
        {
                ...
                while(m_queue.Dequeue (dst, queueEntry))
                {
                        ...
                        if (!tHeader.IsValid ())
                        {
                                NS_LOG_DEBUG ("LGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
...
RoutingProtocol::RecoveryMode(Ipv4Address dst, Ptr<Packet> p, UnicastForwardCallback ucb, Ipv4Header header){
        ...
        {
                NS_LOG_DEBUG ("LGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
                NS_LOG_DEBUG ("in recovery recovery-mode meet error");
                return; // drop
        }
        if (tHeader.Get () == LGPSRTYPE_POS)
        {
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
        ...
        {
                NS_LOG_WARN ("LGPSR does not work with more then one address per each interface.");
        }
        ...
        NS_ASSERT (socket != 0);
        socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLGPSR, this));
        socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), LGPSR_PORT));
...
//接受到socket的包
void
RoutingProtocol::RecvLGPSR (Ptr<Socket> socket)
{
        NS_LOG_FUNCTION (this << socket);
        Address sourceAddress;
        Ptr<Packet> packet = socket->RecvFrom (sourceAddress);
        //RemoveHeader
        // TypeHeader tHeader1 (LGPSRTYPE_HELLO);
        ...
        TypeHeader tHeader (LGPSRTYPE_HELLO);
        packet->RemoveHeader (tHeader);
        if (!tHeader.IsValid ())
        {
                NS_LOG_DEBUG ("LGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                NS_LOG_DEBUG ("RecvLGPSR meet error");
                return;
        }
//追加
        HelloHeader hdr;
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("LGPSR does not work with more then one address per each interface. Ignore added address");
        }
}
//删除出地址
void
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("Remove address not participating in LGPSR operation");
        }
}
...
//追加終了      
        ...
        for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
        {
                ...
                socket->SendTo (packet, 0, InetSocketAddress (destination, LGPSR_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
        ...
        NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid LGPSR source address not found");
