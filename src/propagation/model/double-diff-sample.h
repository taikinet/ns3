...
#ifndef DOUBLE_DIFFRACTION_LOSS_MODEL_H
#define DOUBLE_DIFFRACTION_LOSS_MODEL_H
...
class DoubleDiffSample : public PropagationLossModel
{
public:
  /**
...
private:
  ...
  // 交差判定 
	...
	double DoCalcTwoRayGroundLossRev( Ptr<MobilityModel> a, Ptr<MobilityModel> b) const;
  ...
  // 標高計算
  void DoCalcElevation(VegeDataexp& vstruct)const;
  
  
  // ルート判定
  void DeterminePath(VegeDataexp& vstrut)const;
  
  // 
  void DetermineArea(VegeDataexp&  vstruct)const;
  
  // 
  double DoCalcVegetationalDistance(VegeDataexp&  vstruct_a, VegeDataexp&  vstruct_b)const;
