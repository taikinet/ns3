...
NS_LOG_COMPONENT_DEFINE ("DoubleDiffSample");

NS_OBJECT_ENSURE_REGISTERED (DoubleDiffSample);


TypeId 
DoubleDiffSample::GetTypeId (void){
  static TypeId tid = TypeId ("ns3::DoubleDiffSample")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<DoubleDiffSample> ()
    .AddAttribute ("Frequency",
                   "The carrier frequency (in Hz) at which propagation occurs  (default is 900MHz).",
                   DoubleValue (142000000),
                   MakeDoubleAccessor (&DoubleDiffSample::SetFrequency,
                                       &DoubleDiffSample::GetFrequency),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SystemLoss", "The system loss",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&DoubleDiffSample::m_systemLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MinLoss", 
                   "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&DoubleDiffSample::SetMinLoss,
                                       &DoubleDiffSample::GetMinLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("M1V0", 
                   "Vector 0 of Obstacle",			// 山１端
                   Vector3DValue (Vector3D(-3801883.204, 3469896.97, 3756478.651)),
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM1V0),
                   MakeVector3DChecker ())							// MakeDoubleChackerのように<>で型指定はしない（するとエラーになる)
    .AddAttribute ("M1V1", 
                   "Vector 1 of Obstacle",			// 山１頂点
                   Vector3DValue (Vector3D(-3801630.647, 3469592.819, 3758218.86)),	
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM1V1),
                   MakeVector3DChecker ())
    .AddAttribute ("M1V2", 
                   "Vector 2 of Obstacle",			// 計算から求めた
                   Vector3DValue (Vector3D(-3801207.126392, 3469206.289061, 3757800.175681)),
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM1V2),
                   MakeVector3DChecker ())
    .AddAttribute ("M2V0", 
                   "Vector 0 of Obstacle",			// 山２端
                   Vector3DValue (Vector3D(-3801990.028, 3469773.17, 3756513.565)),			// 初期値はVector3D(x, y, z)で指定する 
                   //Vector3DValue (Vector3D(-3801883.204, 3469896.97, 3756478.651)),
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM2V0),
                   MakeVector3DChecker ())							// MakeDoubleChackerのように<>で型指定はしない（するとエラーになる)
    .AddAttribute ("M2V1", 
                   "Vector 1 of Obstacle",			// 山２頂点
                   Vector3DValue (Vector3D(-3802374.742, 3469067.863, 3758018.369)),
                   //Vector3DValue (Vector3D(-3801630.647, 3469592.819, 3758218.86)),		
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM2V1),
                   MakeVector3DChecker ())
    .AddAttribute ("M2V2", 
                   "Vector 2 of Obstacle",			// 計算から求めた
                   Vector3DValue (Vector3D(-3801937.431134, 3468668.885736, 3757586.159557)),
                   //Vector3DValue (Vector3D(-3801207.126392, 3469206.289061, 3757800.175681)),
                   MakeVector3DAccessor (&DoubleDiffSample::SetVectorM2V2),
                   MakeVector3DChecker ())
    .AddAttribute ("AntennaHeight",
                   "The height of the antenna (m) ",
                   DoubleValue (1.5),
                   MakeDoubleAccessor (&DoubleDiffSample::AntennaHeight),
                   MakeDoubleChecker<double> ())
  ...
  // コンストラクタ
DoubleDiffSample::DoubleDiffSample (){
}
...
// システム損失のセット
void
DoubleDiffSample::SetSystemLoss (double systemLoss){					    
  m_systemLoss = systemLoss;
}

// システム損失の返却
double
DoubleDiffSample::GetSystemLoss (void) const{
  return m_systemLoss;
}

// 最小損失のセット
void
DoubleDiffSample::SetMinLoss (double minLoss){
  m_minLoss = minLoss;
}

// 最小損失の返却
double
DoubleDiffSample::GetMinLoss (void) const{
  return m_minLoss;
}

// 周波数と波長のセット
void
DoubleDiffSample::SetFrequency (double frequency){
...
// 周波数の返却
double
DoubleDiffSample::GetFrequency (void) const{
  return m_frequency;
}
...
// 障害物のベクトルM1VOのセット
void
DoubleDiffSample::SetVectorM1V0 (Vector3D m1v0 ){
	M1V0.x = m1v0.x;
	M1V0.y = m1v0.y;
	M1V0.z = m1v0.z;
}
// 障害物のベクトルM1V1のセット
void
DoubleDiffSample::SetVectorM1V1 (Vector3D m1v1 ){
	M1V1.x = m1v1.x;
	M1V1.y = m1v1.y;
	M1V1.z = m1v1.z;
}
...
// 障害物のベクトルM1V2のセット
void
DoubleDiffSample::SetVectorM1V2 (Vector3D m1v2 ){
	M1V2.x = m1v2.x;
	M1V2.y = m1v2.y;
	M1V2.z = m1v2.z;
}
...
// 障害物のベクトルM2V0のセット
void
DoubleDiffSample::SetVectorM2V0 (Vector3D m2v0 ){
	M2V0.x = m2v0.x;
	M2V0.y = m2v0.y;
	M2V0.z = m2v0.z;
}
...
// 障害物のベクトルM2V1のセット
void
DoubleDiffSample::SetVectorM2V1 (Vector3D m2v1 ){
	M2V1.x = m2v1.x;
	M2V1.y = m2v1.y;
	M2V1.z = m2v1.z;
}
...
// 障害物のベクトルM2V2のセット
void
DoubleDiffSample::SetVectorM2V2 (Vector3D m2v2 ){
	M2V2.x = m2v2.x;
	M2V2.y = m2v2.y;
	M2V2.z = m2v2.z;
}
...
void
DoubleDiffSample::SetAnntenaHeight (double aHeight){
	AntennaHeight = aHeight;
}

// dBm(デシベルミリ) → W(ワット)変換
double
DoubleDiffSample::DbmToW (double dbm) const{
...
// W(ワット) → dBm(デシベルミリ)変換
double
DoubleDiffSample::DbmFromW (double w) const{
...
// Friisの自由空間損失の計算
double
DoubleDiffSample::DoCalcFriisLoss (Ptr<MobilityModel> a,
  ...
  // 平面大地2波反射モデルによる損失
double
DoubleDiffSample::DoCalcTwoRayGroundLoss( Ptr<MobilityModel> a,
                                                 Ptr<MobilityModel> b) const{
  double distance = a->GetDistanceFrom (b);
  if (distance <= 0.5)
    {
 ...
 // 平面大地改定版
double
DoubleDiffSample::DoCalcTwoRayGroundLossRev( Ptr<MobilityModel> a,
                                                 Ptr<MobilityModel> b) const{
...
//引数n1は送信点、n2は回折点
Vector3D
DoubleDiffSample::DoCalcVerticalTwoPoint(Vector3D n1, Vector3D n2) const{
...
/*
double
DoubleDiffSample::DoCalcDiffractionLoss (Ptr<MobilityModel> n1,
	...
	{
		//山１との回折点d1
		Vector3D d1  (directpoint(M1V0,M1V1,A));
		//std::cout << "回折点d1(" << std::setw(7) << d1.x << ", " << std::setw(7) << d1.y << ", " << std::setw(7) << d1.z << ")" << std::endl;
		//山2とd1で交差判定
		...
		{	
			f=3;
			Vector3D d2  (directpoint(M2V0,M2V1,d1));
	...
	{
		f = 2;
		Vector3D d2 (directpoint(M2V0,M2V1,A));
		...
		{
			f=3;
			Vector3D d1 (directpoint(M1V0,M1V1,d2));
		...
		{
			f=1;//山１のみで回折
			Vector3D d1 (directpoint(M1V0,M1V0,A));
...
/*
double
DoubleDiffSample::DoCalcDiffRev(Ptr<MobilityModel> n1,
											     Ptr<MobilityModel> n2)const{
	int f = 0;//交差判定用フラグ、0が回折しない、１が山１でのみ回折、２が山２のみで回折、３が二重回折
	// 通信ノードの座標ベクトルの取得
	Vector3D A = n1->GetPosition();
...
double
DoubleDiffSample::DoCalcDiffLossRev(Ptr<MobilityModel> n1,
											     Ptr<MobilityModel> n2)const{
...
//山一つとの回折の計算
double
DoubleDiffSample::DoCalcSingleDiffLoss(Vector3D n1, Vector3D n2, Vector3D d)const{
	
	double h = height(n1,n2,d);
...
//二重回折の計算
//計算式の詳細は2018/10/22ゼミ資料参照
double
DoubleDiffSample::DoCalcDoubleDiffLoss(Vector3D s,Vector3D d1,Vector3D d2,Vector3D r)const{
  Vector3D h1,h2;
  h1.x = d1.x;
  h1.y = d1.y;
  h2.x = d2.x;
...
//ナイフエッジの減衰を計算する。
double 
DoubleDiffSample::DoCalcKnifeEdgeLoss (double d1, double d2, double h)const {
...
Vector3D
DoubleDiffSample::CrossingDetection01 (Vector3D A,Vector3D B) const{
...
//引数Aは送信ノードの座標、引数Bは受信ノードの座標
Vector3D
DoubleDiffSample::CrossingDetection02 (Vector3D A,Vector3D B) const{
	Vector3D IntersectionPoint (0.0, 0.0, 0.0);
	// 交差判定フラグ
	bool C_Detection = false;
...
Vector3D
DoubleDiffSample::DoCalcVerticalPoint (Ptr<MobilityModel> a,
	...
	double tmp1 = pow((B.x-A.x),2)+pow((B.y-A.y),2)+pow((A.z-B.z),2);
	double tmp2 = (I.x-A.x)*(B.x-A.x)+(I.y-A.y)*(B.y-A.y)+(I.z-A.z)*(B.z-A.z);
	double tmpt = tmp2/tmp1;
	IntersectionPoint.x = A.x + tmpt*(A.x-B.x);
	IntersectionPoint.y = A.y + tmpt*(A.y-B.y);
	IntersectionPoint.z = A.z + tmpt*(A.z-B.z);
...
//2直線の交点を求める
Vector3D
DoubleDiffSample::DoCalcCrossingPoint (Ptr<MobilityModel> a, Vector3D L1, Vector3D X, Vector3D L2) const{
	...
	Vector3D A = a->GetPosition();
	//AL2ベクトルの作成
	Vector3D AL2 (L2.x - A.x, L2.y - A.y, L2.z - A.z);	
	//XL1ベクトルの作成
	Vector3D XL1 (L1.x - X.x, L1.y - X.y, L1.z - X.z);
	
	//AL2ベクトルとXL1ベクトルの単位ベクトルの作成
	Vector3D n1 (get_unit_vector(AL2));
	Vector3D n2 (get_unit_vector(XL1));
    ...
    //if( work2 == 0 ) { detection = false ; }
    
    Vector3D AX (X.x - A.x, X.y - A.y, X.z - A.z);
        
    double d1 = ( dot_product(AX, n1 ) - work1 * dot_product( AX, n2 ) ) / work2;
    double d2 = ( work1 * dot_product(AX, n1 ) - dot_product( AX, n2 ) ) / work2;
    ...
    std::cout << "2点間の距離"<< dist_vertex_np(tmp1,tmp2)<< " (m)" << std::endl;
    
    if(dist_vertex_np(tmp1,tmp2)<0.0001){
		IntersectionPoint.x=tmp1.x;
		IntersectionPoint.y=tmp1.y;
		IntersectionPoint.z=tmp1.z;
		return IntersectionPoint;
	}
	else{
		std::cout << "直線は交差しませんでした" << std::endl;		
		IntersectionPoint.x=tmp1.x;
		IntersectionPoint.y=tmp1.y;
		IntersectionPoint.z=tmp1.z;
		return IntersectionPoint;
...
//単位ベクトルを求める関数
Vector3D
DoubleDiffSample::get_unit_vector (Vector3D v) const{
	 //ベクトルの長さ
...
//内積
double 
DoubleDiffSample::dot_product( Vector3D vl,  Vector3D vr) const {
    return vl.x * vr.x + vl.y * vr.y + vl.z * vr.z;
}

//２点間の距離
double 
DoubleDiffSample::dist_vertex_np(Vector3D v1, Vector3D v2) const{
...
//2点→ベクトル変換abベクトルの作成
Vector3D
DoubleDiffSample::get_vector(Vector3D a, Vector3D b) const{
	Vector3D New;
	New.x = b.x-a.x;
	New.y = b.y-a.y;
	New.z = b.z-a.z;
...
//3点の高さを求める
//点bから辺acに向かって伸びる垂線
double DoubleDiffSample::height(Vector3D a, Vector3D b, Vector3D c) const{
	//三角形の面積の公式
...
//回折点をもとめるメソッド
//引数は山の辺を構成する２点の座標a,bと送信点ノード座標p
Vector3D
DoubleDiffSample::directpoint(Vector3D a,Vector3D b,Vector3D p) const{
	Vector3D AB = get_vector(a,b);
	Vector3D AP = get_vector(a,p);
	double length_AB = pow(dist_vertex_np(a,b),2);
	//内積÷ABの長さの２乗
	double i = dot_product(AB,AP)/length_AB;
	Vector3D result;
	result.x = a.x+i*AB.x;
	result.y = a.y+i*AB.y;
	result.z = a.z+i*AB.z;
...
Vector3D
DoubleDiffSample::watanabe1(Vector3D p) const{
...
Vector3D
DoubleDiffSample::watanabe2(Vector3D p) const{
...
// V-1 標高計算メソッド(近似式ver)
void
DoubleDiffSample::DoCalcElevation(VegeDataexp& vstruct)const
{
...
// V-2 ルート判定メソッド
void
DoubleDiffSample::DeterminePath(VegeDataexp& vstruct)const
{
	// 内側法線ベクトルN
	// あとで属性に登録しておく
...
// V-3 植生判定メソッド
void
DoubleDiffSample::DetermineArea(VegeDataexp& vstruct)const
{
	// 森林限界
...
double
DoubleDiffSample::DoCalcVegetationalDistance(VegeDataexp& vstruct_a, VegeDataexp& vstruct_b)const
{
	// 植生による減衰をうける距離
	double Vdistance = 0;
	// 森林限界
...
// 植生減衰を計算するメソッド
double
DoubleDiffSample::DoCalcVegetationalLoss(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
	
	VegeDataexp vstructA;
	VegeDataexp vstructB;
...
// 受信電力の計算
double
DoubleDiffSample::DoCalcRxPower (double txPowerDbm,
                                          Ptr<MobilityModel> a,
                                          Ptr<MobilityModel> b) const{
	// 最終的な損失
	double SumLoss = 0.0;
	...
	TwoRayRev = DoCalcTwoRayGroundLossRev(a,b);
	vAttenuation = DoCalcVegetationalLoss(a,b); //植生による減衰
	//rev = DoCalcDiffRev(a,b);
	diffrev = DoCalcDiffLossRev(a,b);
	

	
	
	//std::cout << "従来回折損" << DiffLoss << " (dB)" << std::endl;
	//std::cout << "改定回折損" << rev << " (dB)" << std::endl;
	std::cout << "平面大地による損失" << TwoRayLoss << " (dB)" << std::endl;
	std::cout << "改定版平面大地による損失" << TwoRayRev << " (dB)" << std::endl;
	//std::cout << "植生による損失" << vAttenuation << " (dB)" << std::endl;
	
	
	SumLoss = TwoRayLoss+vAttenuation;

	return txPowerDbm-SumLoss+diffrev;
   
}

// PropagationLossModelを親クラスにもつ場合は必ず実装するメソッド
int64_t
DoubleDiffSample::DoAssignStreams (int64_t stream){
  return 0;
}
