...
#include "mountain-propagation-loss-model-rev-forD.h"
...
NS_LOG_COMPONENT_DEFINE ("MountainPropagationLossModelRevForD");

NS_OBJECT_ENSURE_REGISTERED (MountainPropagationLossModelRevForD);


TypeId 
MountainPropagationLossModelRevForD::GetTypeId (void){
  static TypeId tid = TypeId ("ns3::MountainPropagationLossModelRevForD")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<MountainPropagationLossModelRevForD> ()
    .AddAttribute ("Frequency",
                   "The carrier frequency (in Hz) at which propagation occurs  (default is 900MHz).",
                   DoubleValue (142000000),
                   MakeDoubleAccessor (&MountainPropagationLossModelRevForD::SetFrequency,
                                       &MountainPropagationLossModelRevForD::GetFrequency),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SystemLoss", "The system loss",
                   DoubleValue (1.0),
                   MakeDoubleAccessor (&MountainPropagationLossModelRevForD::m_systemLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("MinLoss", 
                   "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                   DoubleValue (0.0),
                   MakeDoubleAccessor (&MountainPropagationLossModelRevForD::SetMinLoss,
                                       &MountainPropagationLossModelRevForD::GetMinLoss),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("M1V0", 
                   "Vector 0 of Obstacle",			// AD006
                   Vector3DValue (Vector3D(-3802056.926, 3470149.789, 3756099.022)),
                   //Vector3DValue (Vector3D(0, 0, 0)),
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM1V0),
    ...
    .AddAttribute ("M1V1", 
                   ...
                   //Vector3DValue (Vector3D(0, 0, 0)),
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM1V1),
                   MakeVector3DChecker ())
    .AddAttribute ("M1V2", 
                   "Vector 2 of Obstacle",			// 計算から求めた
                   Vector3DValue (Vector3D(-3803596.554,3468518.125,3755869.378)),
                   //Vector3DValue (Vector3D(0,0,0)),
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM1V2),
                   MakeVector3DChecker ())
    .AddAttribute ("M2V0", 
                   ...
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM2V0),
                   MakeVector3DChecker ())							// MakeDoubleChackerのように<>で型指定はしない（するとエラーになる)
    .AddAttribute ("M2V1", 
                   ...
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM2V1),
                   MakeVector3DChecker ())
    .AddAttribute ("M2V2", 
                   "Vector 2 of Obstacle",			// 計算から求めた
                   ...
                   MakeVector3DAccessor (&MountainPropagationLossModelRevForD::SetVectorM2V2),
                   MakeVector3DChecker ())
    .AddAttribute ("AntennaHeight",
                   "The height of the antenna (m) ",
                   DoubleValue (1.5),
                   MakeDoubleAccessor (&MountainPropagationLossModelRevForD::AntennaHeight),
                   MakeDoubleChecker<double> ())
...
// コンストラクタ
MountainPropagationLossModelRevForD::MountainPropagationLossModelRevForD ()
{
}

// システム損失のセット
void
MountainPropagationLossModelRevForD::SetSystemLoss (double systemLoss)
{					    
...
// システム損失の返却
double
MountainPropagationLossModelRevForD::GetSystemLoss (void) const
{
  return m_systemLoss;
}
...
// 最小損失のセット
void
MountainPropagationLossModelRevForD::SetMinLoss (double minLoss)
{
  m_minLoss = minLoss;
}
...
// 最小損失の返却
double
MountainPropagationLossModelRevForD::GetMinLoss (void) const
{
  return m_minLoss;
}
...
// 周波数と波長のセット
void
MountainPropagationLossModelRevForD::SetFrequency (double frequency)
{
...
// 周波数の返却
double
MountainPropagationLossModelRevForD::GetFrequency (void) const
{
  return m_frequency;
}
...
// 障害物のベクトルM1VOのセット
void
MountainPropagationLossModelRevForD::SetVectorM1V0 (Vector3D m1v0 ){
	M1V0.x = m1v0.x;
	M1V0.y = m1v0.y;
	M1V0.z = m1v0.z;
}
// 障害物のベクトルM1V1のセット
void
MountainPropagationLossModelRevForD::SetVectorM1V1 (Vector3D m1v1 ){
	M1V1.x = m1v1.x;
	M1V1.y = m1v1.y;
	M1V1.z = m1v1.z;
}
...
// 障害物のベクトルM1V2のセット
void
MountainPropagationLossModelRevForD::SetVectorM1V2 (Vector3D m1v2 ){
	M1V2.x = m1v2.x;
	M1V2.y = m1v2.y;
	M1V2.z = m1v2.z;
}
...
// 障害物のベクトルM2V0のセット
void
MountainPropagationLossModelRevForD::SetVectorM2V0 (Vector3D m2v0 ){
	M2V0.x = m2v0.x;
	M2V0.y = m2v0.y;
	M2V0.z = m2v0.z;
}
...
// 障害物のベクトルM2V1のセット
void
MountainPropagationLossModelRevForD::SetVectorM2V1 (Vector3D m2v1 ){
	M2V1.x = m2v1.x;
	M2V1.y = m2v1.y;
	M2V1.z = m2v1.z;
}
...
// 障害物のベクトルM2V2のセット
void
MountainPropagationLossModelRevForD::SetVectorM2V2 (Vector3D m2v2 ){
	M2V2.x = m2v2.x;
	M2V2.y = m2v2.y;
	M2V2.z = m2v2.z;
}
void
MountainPropagationLossModelRevForD::SetAnntenaHeight (double aHeight)
{
...
// dBm(デシベルミリ) → W(ワット)変換
double
MountainPropagationLossModelRevForD::DbmToW (double dbm) const
{
...
// W(ワット) → dBm(デシベルミリ)変換
double
MountainPropagationLossModelRevForD::DbmFromW (double w) const
{
...
// Friisの自由空間損失の計算
double
MountainPropagationLossModelRevForD::DoCalcFriisLoss (Ptr<MobilityModel> a,
                                          Ptr<MobilityModel> b) const
{
  ...
  // 平面大地2波反射モデルによる損失
double
MountainPropagationLossModelRevForD::DoCalcTwoRayGroundLoss( Ptr<MobilityModel> a,
                                                 Ptr<MobilityModel> b) const
{
  double distance = a->GetDistanceFrom (b);
  ...
  if(A.x== -3800304.117 && A.y==3473142.429&& A.z==3754535.257){
	Aantenna=50.0;
	if(B.x==-3800304.117  && B.y==3473142.429 && B.z==3754535.257)
	Cantenna=50;
	else Cantenna=50;
	}
	else{
  ...
  if (distance <= 0.5)
    ...
    //std::cout << "Bのアンテナ高" << Cantenna << " m" << std::endl;
  // ブレークポイント
  ...
  if (distance <= dCross)
  ...
  else   // 平面大地２波反射モデル
    {
	  ...
	  // 分母
	  double denominator = Aantenna * Cantenna;
	  // 損失
...
*/
double
MountainPropagationLossModelRevForD::DoCalcDiffRev(Ptr<MobilityModel> n1,
											     Ptr<MobilityModel> n2)const{
...
//山一つとの回折の計算
double
MountainPropagationLossModelRevForD::DoCalcSingleDiffLoss(Vector3D n1, Vector3D n2, Vector3D d)const{
...
//二重回折の計算
double
MountainPropagationLossModelRevForD::DoCalcDoubleDiffLoss(Vector3D s,Vector3D d1,Vector3D d2,Vector3D r)const{
  Vector3D h1,h2;
  h1.x = d1.x;
  h1.y = d1.y;
  h2.x = d2.x;
...
//二重回折の計算
double
MountainPropagationLossModelRevForD::DoCalcDoubleDiffLoss(Vector3D s,Vector3D d1,Vector3D f1,Vector3D r,Vector3D d2,Vector3D f2)const{
 
 
	//a,b,cを求める準備、回折点から半直線SRに対し垂直な点を求める
...
//ナイフエッジの減衰を計算する。
double 
MountainPropagationLossModelRevForD::DoCalcKnifeEdgeLoss (double d1, double d2, double h)const {
...
Vector3D
MountainPropagationLossModelRevForD::CrossingDetection01 (Vector3D A,Vector3D B) const{
...
//引数Aは送信ノードの座標、引数Bは受信ノードの座標
Vector3D
MountainPropagationLossModelRevForD::CrossingDetection02 (Vector3D A,Vector3D B) const{
	Vector3D IntersectionPoint (0.0, 0.0, 0.0);
	// 交差判定フラグ
	bool C_Detection = false;
...
//単位ベクトルを求める関数
Vector3D
MountainPropagationLossModelRevForD::get_unit_vector (Vector3D v) const{
	 //ベクトルの長さ
...
//内積
double 
MountainPropagationLossModelRevForD::dot_product( Vector3D vl,  Vector3D vr) const {
    return vl.x * vr.x + vl.y * vr.y + vl.z * vr.z;
}

//２点間の距離
double 
MountainPropagationLossModelRevForD::dist_vertex_np(Vector3D v1, Vector3D v2) const{
...
//2点→ベクトル変換abベクトルの作成
Vector3D
MountainPropagationLossModelRevForD::get_vector(Vector3D a, Vector3D b) const{
	Vector3D New;
	New.x = b.x-a.x;
	New.y = b.y-a.y;
	New.z = b.z-a.z;
...
//点bから辺acに向かって伸びる垂線
double 
MountainPropagationLossModelRevForD::height(Vector3D a, Vector3D b, Vector3D c) const{
	//三角形の面積の公式
...
Vector3D
MountainPropagationLossModelRevForD::watanabe1(Vector3D p) const{
...
Vector3D
MountainPropagationLossModelRevForD::watanabe2(Vector3D p) const{
...
//戻り値は点のベクトル
Vector3D
MountainPropagationLossModelRevForD::watanabe(Vector3D a, Vector3D b, Vector3D c)const{
...
// V-1 標高計算メソッド(近似式ver)
void
MountainPropagationLossModelRevForD::DoCalcElevation(VegetationDataRevForD& vstruct)const
{
...
// V-2 ルート判定メソッド
void
MountainPropagationLossModelRevForD::DeterminePath(VegetationDataRevForD& vstruct)const
{
	// 内側法線ベクトルN
	// あとで属性に登録しておく
...
// V-3 植生判定メソッド
void
MountainPropagationLossModelRevForD::DetermineArea(VegetationDataRevForD& vstruct)const
{
	// 森林限界
...
double
MountainPropagationLossModelRevForD::DoCalcVegetationalDistance(VegetationDataRevForD& vstruct_a, VegetationDataRevForD& vstruct_b)const
{
	// 植生による減衰をうける距離
	double Vdistance = 0;
	// 森林限界
...
// 植生減衰を計算するメソッド
double
MountainPropagationLossModelRevForD::DoCalcVegetationalLoss(Ptr<MobilityModel> a, Ptr<MobilityModel> b) const
{
	
	VegetationDataRevForD vstructA;
	VegetationDataRevForD vstructB;
...
// 受信電力の計算
double
MountainPropagationLossModelRevForD::DoCalcRxPower (double txPowerDbm,
                                          Ptr<MobilityModel> a,
                                          Ptr<MobilityModel> b) const
{
	// 最終的な損失
...
int64_t
MountainPropagationLossModelRevForD::DoAssignStreams (int64_t stream)
{
  return 0;
}
