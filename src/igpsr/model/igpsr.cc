...
#define IGPSR_LS_GOD 0

#define IGPSR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("IgpsrRoutingProtocol");
...
// TODO read ns3 api
        ...
        {
                static TypeId tid = TypeId ("ns3::igpsr::DeferredRouteOutputTag").SetParent<Tag> (); //设置parent的tid,获取<Tag>是模版
                return tid;
        }
...
#define IGPSR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-IGPSR_MAXJITTER)-IGPSR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (x->GetValue (-IGPSR_MAXJITTER, IGPSR_MAXJITTER)))
#define FIRST_JITTER (Seconds (x->GetValue (0, IGPSR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for IGPSR control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::IGPSR_PORT = 666;
...
{
        static TypeId tid = TypeId ("ns3::igpsr::RoutingProtocol")
                            ...
                            .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                                           EnumValue (IGPSR_LS_GOD),
                                           MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                                           MakeEnumChecker (IGPSR_LS_GOD, "GOD",
                                                            IGPSR_LS_RLS, "RLS"))
                                  ...
                                  LocalDeliverCallback lcb, ErrorCallback ecb)
{
        ...
        {
                NS_LOG_LOGIC ("No igpsr interfaces");
                return false;
        }
        NS_ASSERT (m_ipv4 != 0);
        NS_ASSERT (p != 0);
        ...
        // TypeHeader tHeader (IGPSRTYPE_POS);
        // //去掉packet的包头
        ...
        if (m_ipv4->IsDestinationAddress (dst, iif))
        {
                ...
                if (!tHeader.IsValid ())
                {
                        NS_LOG_DEBUG ("IGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                        NS_LOG_DEBUG ("RoutingInput meet error");
                        return false;
                }
                NS_LOG_DEBUG ("Received packet");
                //如果是POS的包，就把POS的包头再去掉，所以不需要了解pos的信息了，直接去掉）
                if (tHeader.Get () == IGPSRTYPE_POS)
                {
                        PositionHeader phdr;
                              ...
                              Ptr<NetDevice> oif, Socket::SocketErrno &sockerr)
{
        ...
        else
        {
                ...
                nextHop = m_neighbors.BestNeighbor (dstPos, myPos);//追加
        }
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
        ...
        if(recovery)
        {
                ...
                while(m_queue.Dequeue (dst, queueEntry))
                {
                        ...
                        if (!tHeader.IsValid ())
                        {
                                NS_LOG_DEBUG ("IGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
        ...
        {
                NS_LOG_WARN ("IGPSR does not work with more then one address per each interface.");
        }
        ...
        NS_ASSERT (socket != 0);
        socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvIGPSR, this));
        socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), IGPSR_PORT));
...
//接受到socket的包
void
RoutingProtocol::RecvIGPSR (Ptr<Socket> socket)
{
        NS_LOG_FUNCTION (this << socket);
        Address sourceAddress;
        Ptr<Packet> packet = socket->RecvFrom (sourceAddress);
        //RemoveHeader
        // TypeHeader tHeader1 (IGPSRTYPE_HELLO);
        ...
        TypeHeader tHeader (IGPSRTYPE_HELLO);
        packet->RemoveHeader (tHeader);
        if (!tHeader.IsValid ())
        {
                NS_LOG_DEBUG ("IGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                NS_LOG_DEBUG ("RecvIGPSR meet error");
                return;
        }
        ...
        uint64_t m_m1HopNumNeighbor;//追加
        m_m1HopNumNeighbor = hdr.GetPNumNeighbor();//追加
        InetSocketAddress inetSourceAddr = InetSocketAddress::ConvertFrom (sourceAddress);
        Ipv4Address sender = inetSourceAddr.GetIpv4 ();
        Ipv4Address receiver = m_socketAddresses[socket].GetLocal ();
        NS_LOG_DEBUG("update position"<<Position.x<<Position.y );
        NS_LOG_DEBUG("1hop number"<<m_m1HopNumNeighbor);
        //更新neighbor的信息
        //UpdateRouteToNeighbor (sender, receiver, Position);
        UpdateRouteToNeighbor (sender, receiver, Position, m_m1HopNumNeighbor);//追加
}
...
//追加開始
void
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver, Vector Pos, uint64_t M1HopNumNeighbor)//追加k
{
  //m_neighbors.AddEntry (sender, Pos);
  m_neighbors.AddEntry (sender, Pos, M1HopNumNeighbor);//追加
}
//追加終了
//关闭interface的socket
void
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("IGPSR does not work with more then one address per each interface. Ignore added address");
        }
}
//删除出地址
void
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("Remove address not participating in IGPSR operation");
        }
}
...
RoutingProtocol::SendHello ()
{
        ...
        positionX = MM->GetPosition ().x;
        positionY = MM->GetPosition ().y;
        uint64_t M1HopNumNeighbor=m_neighbors.Get1HopNumNeighbor();//追加
        ...
        for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
        {
                ...
                NS_LOG_DEBUG("Send hello Position"<<positionX<<positionY );
                NS_LOG_DEBUG("Send hello 1 hop number"<<M1HopNumNeighbor);
                HelloHeader helloHeader (((uint64_t) positionX),((uint64_t) positionY),((uint64_t) M1HopNumNeighbor));//追加
                ...
                socket->SendTo (packet, 0, InetSocketAddress (destination, IGPSR_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
        ...
        NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid IGPSR source address not found");
...
RoutingProtocol::AddHeaders (Ptr<Packet> p, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route)
{
        ...
        else
        {
                ...
                nextHop = m_neighbors.BestNeighbor (m_locationService->GetPosition (destination), myPos);//追加
        }
