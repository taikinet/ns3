#include "igpsr-ptable.h"
#include "ns3/simulator.h"
#include "ns3/log.h"
#include <algorithm>
#include <cmath>

NS_LOG_COMPONENT_DEFINE ("IgpsrTable");
...
namespace ns3 {
  namespace igpsr {
    ...
    */
    ...
    //TODO finish velocity
    void
    PositionTable::AddEntry (Ipv4Address id, Vector position, uint64_t t1HopNumNeighbor)//追加
    {
      m_tNumNeighbor=0;//追加
      m_1HopNumNeighbor.insert(std::make_pair(id, t1HopNumNeighbor));//追加
      std::map<Ipv4Address, Metrix >::iterator i = m_table.find (id);
      ...
      if (i != m_table.end () || id.IsEqual (i->first))
      {
        ...
        m_tNumNeighbor=m_table.size();//追加
        return; // 必须返回，否则后面没有办法进行
      }
    ...
    */
    ...
    PositionTable::Clear ()
    {
      m_table.clear ();
      m_tNumNeighbor=0;//追加
      m_1HopNumNeighbor.clear();//追加
    }

    /**
    * \brief Gets next hop according to IGPSR protocol
    ...
    */
    ...
    PositionTable::BestNeighbor (Vector position, Vector nodePos)
    {
      ...
      double initialDistance = CalculateDistance (nodePos, position);//宛先と転送ノードの距離
      double neighborDistance =0;//宛先と隣接ノードとの距離
      double R=174.0;
      if (m_table.empty ())
        {
      ...
      double bestFoundDistance = CalculateDistance (m_table.begin ()->second.position, position);//宛先と隣接ノードとの距離
      //
      bestFoundDistance = (bestFoundDistance-R)+(R/std::sqrt(m_1HopNumNeighbor.at(m_table.begin()->first)+1));//提案

      std::map<Ipv4Address, Metrix >::iterator i;;
      for (i = m_table.begin (); !(i == m_table.end ()); i++)
        {
          neighborDistance=CalculateDistance (i->second.position, position);//宛先と隣接のとの距離
          double candidateDistance=(CalculateDistance (i->second.position, position)-R)+(R/std::sqrt(m_1HopNumNeighbor.at(i->first)+1));//提案
          if (bestFoundDistance >= candidateDistance && initialDistance >= neighborDistance)//順に提案の値を比較していく(低い時)
            {
              bestFoundDistance=candidateDistance;//提案の距離
              bestFoundID = i->first;//その時の隣接ノードID
            }
        }
      ...
      for (i = m_table.begin (); !(i == m_table.end ()); i++)
        {
          //neighborDistance=CalculateDistance (i->second.position, position);//宛先と隣接のとの距離
          double candidateDistance=(CalculateDistance (i->second.position, position)-R)+(R/std::sqrt(m_1HopNumNeighbor.at(i->first)+1));//提案
          if (bestFoundDistance >= candidateDistance && initialDistance > candidateDistance)//順に提案の値を比較していく(低い時)
            {
              bestFoundDistance=candidateDistance;//提案の距離
              bestFoundID = i->first;//その時の隣接ノードID
            }
    ...
    /**
    * \brief Gets next hop according to IGPSR recovery-mode protocol (right hand rule)
    ...
    */
    ...
    //追加開始
    uint64_t PositionTable::Get1HopNumNeighbor()
    {
      return m_tNumNeighbor;
    }
    //追加終了
