...
int64_t 
OnOffApplication::AssignStreams (int64_t stream)//このモデルで使用される確率変数に固定確率変数ストリーム番号を割り当てます。割り当てられたストリームの数 (おそらくゼロ) を返します。
{
  ...
  if (!m_socket)
    {
      ...
      //shinato
      //m_socket->SetRecvCallback (MakeCallback (&OnOffApplication::HandleRead, this));
...
void OnOffApplication::SendPacket ()//データ送信
{
  NS_LOG_FUNCTION (this);
...
//shinato
void OnOffApplication::HandleRead (Ptr<Socket> socket)
{
