...
private:
  ...
  /**
   * \brief 特定の宛先とポートにパケットを送信する(IPv4)
   * \param p packet
  ...
  /**
   * \brief L3 プロトコルが TCP に渡すべき ICMP パケットを受信したときに呼び出される
  ...
  mutable enum SocketErrno m_errno;           //!< ソケットエラーコード
  bool                     m_shutdownSend;    //!< 送信不可になった
  bool                     m_shutdownRecv;    //!< 受信不可になった
  bool                     m_connected;       //!< 接続確立
  bool                     m_allowBroadcast;  //!< ブロードキャストパケットの送信を許可
