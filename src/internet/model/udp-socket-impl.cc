...
UdpSocketImpl::DoSend (Ptr<Packet> p)
{
  ...
  if (Ipv4Address::IsMatchingType (m_defaultAddress))//ピア情報設定
    {
...
UdpSocketImpl::DoSendTo (Ptr<Packet> p, Ipv4Address dest, uint16_t port, uint8_t tos)
{
  ...
  if (dest.IsBroadcast ())
          ...
          if (maski == Ipv4Mask::GetOnes ())
            {
              ...
              m_udp->Send (p->Copy (), addri, dest,
                           m_endPoint->GetLocalPort (), port);//udpl4protocol::send()呼び出し
  ...
  else if (ipv4->GetRoutingProtocol () != 0)
    {
      ...
      route = ipv4->GetRoutingProtocol ()->RouteOutput (p, header, oif, errno_); //経路情報呼び出し
      if (route != 0)
        {
