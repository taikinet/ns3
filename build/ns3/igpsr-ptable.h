#ifndef IGPSR_PTABLE_H
#define IGPSR_PTABLE_H
...
/*
 * \ingroup igpsr
 * \brief Position table used by IGPSR
 */
class PositionTable
{
public:
  ...
  void AddEntry (Ipv4Address id, Vector position, uint64_t t1HopNumNeighbor);//追加
  ...
  Ipv4Address BestNeighbor (Vector position, Vector nodePos);//追加
  ...
  uint64_t Get1HopNumNeighbor();//追加
...
private:
  ...
  uint64_t m_tNumNeighbor;//追加
  std::map<Ipv4Address,uint64_t> m_1HopNumNeighbor;//追加
