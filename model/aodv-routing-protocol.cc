...
// 隣接ノード数を取得する
uint32_t RoutingProtocol::GetNeighborCount() {
    uint32_t neighborcount = m_nb.GetNeighborCount();
...
double RoutingProtocol::GetRemainingBattery() {
    ...
    // BasicEnergySourceを取得
    Ptr<energy::BasicEnergySource> energySource = node->GetObject<energy::BasicEnergySource>();
    if (energySource) {
        // 残バッテリー量を取得
        NS_ASSERT(energySource->GetRemainingEnergy() >= 0.0);
        return energySource->GetRemainingEnergy();
    }
    NS_LOG_WARN("No BasicEnergySource found for this node");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    {
        m_rreqCount++;          // rrqCountをインクリメント
    }

    // Create RREQ header
    RreqHeader rreqHeader;
    rreqHeader.SetDst(dst);         // dstの設定
    ...
    // 宛先のルートがルーティングテーブルにある場合
    if (m_routingTable.LookupRoute(dst, rt))
    {
        if (rt.GetFlag() != IN_SEARCH)      // ルートが探索中でない場合
        {
            ttl = std::min<uint16_t>(rt.GetHop() + m_ttlIncrement, m_netDiameter);  // ttlを設定
        }
        else        // ルートが探索中の場合
        {
            ttl = rt.GetHop() + m_ttlIncrement;
            if (ttl > m_ttlThreshold)           // ttlが閾値を超えた場合
            {
                ttl = m_netDiameter;            // ttlを最大値に設定
            }
        }
        if (ttl == m_netDiameter)               // ttlが最大値の場合
        {
            rt.IncrementRreqCnt();              // RREQカウントをインクリメント
        }
        if (rt.GetValidSeqNo())                 // 有効なシーケンス番号の場合
        {
            rreqHeader.SetDstSeqno(rt.GetSeqNo());  // 宛先のシーケンス番号を設定
        }
        else
        {
    ...
    // RREQヘッダの設定
    if (m_gratuitousReply)      // 周辺ノードにもrrep情報を共有する場合
    {
        rreqHeader.SetGratuitousRrep(true);
    }
    if (m_destinationOnly)      // 中間ノードがrrepを送信するかどうか
    {
        rreqHeader.SetDestinationOnly(true);
    }

    // シークエンスNo.とrreqIDのインクリメントによる識別
    m_seqNo++;
    ...
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;          // ソケットとインターフェースのマップキー構造
        Ipv4InterfaceAddress iface = j->second;

        rreqHeader.SetOrigin(iface.GetLocal());         // 送信元のIPアドレスを設定
        m_rreqIdCache.IsDuplicate(iface.GetLocal(), m_requestId);       // 重複チェック

        Ptr<Packet> packet = Create<Packet>();          // パケットの作成
        SocketIpTtlTag tag;                             // タグの作成
        tag.SetTtl(ttl);                                // ttlの設定
        packet->AddPacketTag(tag);                      // タグの追加     
        packet->AddHeader(rreqHeader);                  // ヘッダの追加
        TypeHeader tHeader(AODVTYPE_RREQ);              // ヘッダのタイプの設定
        packet->AddHeader(tHeader);                     // ヘッダの追加
