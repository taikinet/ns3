...
/* simpleTCP: simulate TCP behaviours with a simple 4-nodes topology.
 * F.Qian, Nov. 2012
 *
 * n0(tcp source) 
 *  \.1
...
#define PROG_DIR "local/exp02/"

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("OnOffTCPwithCwnd");
...
main (int argc, char *argv[])
{
	...
	cmd.AddValue("bandwidth", 
		"bandwidth of bottleneck link(in Kbps)", bandwidth);
	cmd.Parse(argc, argv);
	...
	std::cout << "use " << tcpType << ", bandwidht: " << bandwidth <<std::endl;
	...
	Address srcAddress (InetSocketAddress (ifs1.GetAddress (0), PORT));
        ...
        ftp.SetAttribute ("DataRate", StringValue ("1500Kbps"));
	...
	// make trace file's name
        //std::string afname = std::string(PROG_DIR) + "data/simple-onoff-tcp.tr";
        //p2p2.EnableAsciiAll (ascii.CreateFileStream (afname));

        // make trace file's name
        std::string fname = std::string(PROG_DIR) + "data/" + tcpType + "-onoff-" + bandwidth + ".cwnd";
	...
	q->TraceConnectWithoutContext ("Drop", MakeCallback (&TraceDropedPkt));
  	...
  	// for PCAP Tracing
	//std::string pfname = std::string(PROG_DIR) + "simple-onoff-tcp";
