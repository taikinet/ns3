...
#define PROG_DIR	"src/queue/examples/"
...
*/
  	...
  	uint32_t    maxPackets  = 600;      // Max Packets allowed in the queue
	...
	if (queueType == "RR") { // set ARED queue parameters
		...
		if (modeBytes) {
			Config::SetDefault ("ns3::DropTailQueue::Mode", 
				EnumValue (ns3::Queue::QUEUE_MODE_BYTES));
			Config::SetDefault ("ns3::DropTailQueue::MaxBytes", 
				UintegerValue (maxPackets * pktSize));
		}
	} else {
	...
	Config::SetDefault ("ns3::TcpL4Protocol::SocketType", 
  	...
  	Config::SetDefault ("ns3::TcpSocket::SegmentSize"   , 
		UintegerValue (pktSize*2 - 42));// 42 = headers size
	...
	p2pLeaf->SetDeviceAttribute ("DataRate", 
		StringValue ("10Mbps"));
  	p2pLeaf->SetChannelAttribute ("Delay"   , 
		StringValue ("2ms"));
...
SimClass::LocalTraceStuff()
{
	...
	for(uint32_t i=0; i<dumbbell_net->LeftCount (); i++) {
		sprintf(fname,"%sdata/rr-queue-n%d.cwnd", PROG_DIR, i);
