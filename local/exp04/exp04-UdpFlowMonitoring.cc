...
#define	PROG_DIR	"local/exp04/"

using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("exp04-SimUdpFlowMonitor");
...
main (int argc, char *argv[])
{
	...
	// Install on/off app on all right side nodes
  	OnOffHelper udp ("ns3::UdpSocketFactory", Address ());
  	udp.SetAttribute ("OnTime" , StringValue ("ns3::UniformRandomVariable[Min=3.0|Max=9.00]"));
  	udp.SetAttribute ("OffTime", StringValue ("ns3::UniformRandomVariable[Min=0.1|Max=0.50]"));
	udp.SetAttribute ("DataRate"  , StringValue ("450Kbps"));
	...
	for (uint32_t i = 0; i < dumbbell_net.LeftCount (); i++) {
      		...
      		udp.SetAttribute ("Remote", remoteAddress);
      		source.Add (udp.Install (dumbbell_net.GetLeft (i)));
    	}
	...
	// make trace file's name
        ...
        std::string fname = std::string(PROG_DIR) + "data/exp04-UDP-flowmon.xml";
