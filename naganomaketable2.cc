...
int makeFile(int t){
    ...
    double max_GPSR = 0.0;  // 最初の値で初期化
    double max_NGPSR = 0.0;
    double max_PGPSR = 0.0;
    double max_DGPSR = 0.0;
    double min_GPSR = 1000000000;  // 最初の値で初期化
    double min_NGPSR = 10000000000;
    double min_PGPSR = 10000000000;
    double min_DGPSR = 10000000000;

	
	std::vector<std::string> traceFiles = {"mobility112.tcl", "mobility37_185.tcl", "mobility74_185.tcl", "mobility112_185.tcl", "mobility150_185.tcl", "mobility185.tcl"};    // ここいじる
    for (const std::string& traceFile : traceFiles) {
        ...
        fstream fs;
        fs.open("/home/hry-user/Simulation300/"+traceFile+"/avarage.txt", ios::out | ios::app);
        fs << traceFile << endl;
        ...
        for(int i=1;i<=10;i++){
            ...
            if (i < 9){
                for(int l=1;l<=t;l++){
                    ...
                    std::string Gstr = "/home/hry-user/Simulation300/"+traceFile+"/GPSR/data" +std::to_string(l)+ ".txt";
                    std::string Istr = "/home/hry-user/Simulation300/"+traceFile+"/NGPSR/data" +std::to_string(l)+ ".txt";
                    std::string Hstr = "/home/hry-user/Simulation300/"+traceFile+"/NPGPSR/data" +std::to_string(l)+ ".txt";
                    std::string Dstr = "/home/hry-user/Simulation300/"+traceFile+"/NDGPSR/data" +std::to_string(l)+ ".txt";
                ...
                fstream fs;
                fs.open("/home/hry-user/Simulation300/"+traceFile+"/"+value+".txt", ios::out);
